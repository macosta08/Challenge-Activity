...
const items = async (req, res) => {
  /*
    #swagger.tags = ['Items']
    #swagger.description = 
        'Consulta el endpoint: https://api.mercadolibre.com/sites/MLA/search?q=:query y devuelve los resultados en un 
        formato indicado.<br><br> 
        Es necesario consumir el endpoint https://api.mercadolibre.com/categories/:id_category para construir el arreglo 
        de categorías basado en la categoría que mas resultados obtuvo'
  ...
  */
  ...
  const apiSearchURL = process.env.API_SEARCH || 'https://api.mercadolibre.com/sites/MLA/search';

  try {
    //Llamado al API SEARCH
    const searchResponse = await (await fetch(`${apiSearchURL}?q=${q}`)).json();
    ...
    if (searchResponse.paging.total > 0) {
      ...
      const cat_filters = searchResponse.filters.find((filter) => filter.id === 'category');
      ...
      const idCategory =
        cat_available_filters !== undefined
          ? cat_available_filters.values.sort((a, b) => b.results - a.results)[0].id
...
const item = async (req, res) => {
  /*
    #swagger.tags = ['Items']
    #swagger.description = 
        'Debe consultar los siguientes endpoints:<br><br>https://api.mercadolibre.com/items/:id
        <br>https://api.mercadolibre.com/items/:id/description<br><br>Y devolver los resultados 
        en el formato indicado. <br><br> 
        <b>IMPORTATE:</b> Fue necesario construir el arreglo de categorías para indicar la categoría propia del producto'
    */
  /* 
  ...
  const apiItemsURL = process.env.API_ITEMS || 'https://api.mercadolibre.com/items';

  try {
    //Llamado al API del detalle del ITEM
    const itemDetail = await (await fetch(`${apiItemsURL}/${id}`)).json();

    //Llamado al API de la descripción del ITEM
    const description = await (await fetch(`${apiItemsURL}/${id}/description`)).json();
...
const categoryResponse = async (id) => {
  const apiCategoriesURL = process.env.API_CATEG || 'https://api.mercadolibre.com/categories';
  return await (await fetch(`${apiCategoriesURL}/${id}`)).json();
};
